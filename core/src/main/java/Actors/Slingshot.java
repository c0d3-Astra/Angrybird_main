...
public class Slingshot extends Actor {
    ...
    public Slingshot(World world, Vector2 position, RedBird bird, GameMananger gameMananger) {
        this.world = world;
        this.slingshotPosition = new Vector2(position.x, -0.05f);
        this.bird = bird;
        ...
        trajectorySprite.setSize(10, 10);
        birdInitialPosition = new Vector2(2.5f, 3f);

        slingshotAnchorLeft = new Vector2(slingshotPosition.x - 0.2f, slingshotPosition.y + 1f);
        slingshotAnchorRight = new Vector2(slingshotPosition.x + 0.2f, slingshotPosition.y + 1f);
    ...
    public void setBird(RedBird bird) {
        this.bird = bird;
    }
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        if (isDragging) {
            // End batch rendering to allow ShapeRenderer usage
            batch.end();

            // Render rubber band with ShapeRenderer
            drawRubberBand();

            // Resume batch rendering
            batch.begin();

            // Render trajectory using the batch
...
//    shapeRenderer.getLineWidth();

    // Set the line width to make the rubber band thicker
    Gdx.gl.glLineWidth(5);

    // Draw from slingshot anchors to the bird's current position
    ...
    private void drawTrajectory(Batch batch) {
        if (launchVelocity == null) {
            return; // No trajectory to draw if launchVelocity is not set
        }
